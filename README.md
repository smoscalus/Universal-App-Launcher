...
Застосунок розділений на два автономні процеси:
- Backend (C++ & Crow) — високопродуктивний веб-сервер, що відповідає за бізнес-логіку, роботу з базою даних HELL NAH та пряму взаємодію з ОС (запуск процесів).

- Frontend (Electron & UI) — графічна оболонка. Electron виступає в ролі браузера та вікна застосунку, взаємодіючи з бекендом через локальні HTTP-запити (REST API).
...
│   ├── /application         # Бізнес-логіка (Services)
│   ├── /domain              # Сутності (Resource, Category)
│   └── /infrastructure      # Реалізація HELL NAH DB та системні виклики
│
├── /electron
│   ├── main.js              # Життєвий цикл вікна Electron
│   └── /renderer            # Фронтенд (HTML, CSS, JS)
│
├── /hellnah                 # Ісходний код власної бази даних
│
...
```
C++ — основна логіка та сервіси.
Crow — мікрофреймворк для створення REST API на C++.
HELL NAH DB — власна вбудована база даних на базі бінарних структур.
Electron — десктопна оболонка.
HTML5/JS — інтерфейс користувача.
```
